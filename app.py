...
<style>
    ...
    .api-warning { background: #451a03; border: 2px solid #f59e0b; color: #fbbf24; padding: 15px; border-radius: 12px; text-align: center; margin: 10px 0; }
</style>
""", unsafe_allow_html=True)

# FIX: Refresh ogni 15 minuti (900 secondi) invece di 30
REFRESH_INTERVAL = 900  # 15 minuti = 900 secondi
...
# Calcolo uso API
calls_per_scan = len(PAIRS)
max_scans_per_day = 800 // calls_per_scan  # ~200 scansioni al giorno
minutes_between_scans = (24 * 60) // max_scans_per_day  # ~7 minuti
...
def get_price(symbol):
    """Prezzo con gestione errore 429"""
    ...
    try:
        ...
        if data.get('code') == 429:
            st.session_state['api_error'] = "Limite API giornaliero superato"
            return None, "API Limit"
...
with col1:
    st.markdown(f"<p class='timestamp'>⏱️ Auto-refresh: ogni {REFRESH_INTERVAL//60} minuti</p>", unsafe_allow_html=True)
with col2:
    st.markdown(f"<p class='timestamp'>📊 API: ~{calls_per_scan} chiamate/scan | Max ~{max_scans_per_day} scansioni/giorno</p>", unsafe_allow_html=True)

# Warning API
if st.session_state['api_error']:
    st.markdown(f"""
        <div class="api-warning">
            <h4>⚠️ {st.session_state['api_error']}</h4>
            <p>Il refresh automatico è stato ridotto a 15 minuti per rispettare i limiti.</p>
            <p>Piano consigliato: <a href="https://twelvedata.com/pricing" target="_blank">Upgrade Twelve Data</a></p>
        </div>
    """, unsafe_allow_html=True)
...
# Toggle auto-analisi
auto_check = st.checkbox(f"🔔 Scansione automatica (ogni {REFRESH_INTERVAL//60} min)", value=True)
...
if st.button("🚀 SCANSIONA TUTTE LE COPPIE", type="primary", use_container_width=True) or auto_check:
    ...
    with st.spinner("📡 Recupero prezzi..."):
        for pair in PAIRS:
    ...
    # Display TUTTE le coppie
    st.subheader("📊 Dashboard Coppie")
    
    if not st.session_state['signals']:
        st.warning("⚠️ Nessun dato disponibile. Controlla la tua API key o attendi il reset giornaliero.")
    else:
        cols = st.columns(2)
